...
namespace paa {
    ...
    public:
        GuiText(const std::string& text) : _text(text) {}
    ...
    public:
        InputText(const std::string& text, const std::string& label, const size_t maxSize=512)
            : _label(label)
        {
    ...
    public:
        template<typename T, typename ...Args,
            typename std::enable_if<std::is_base_of<GuiObject, T>::value, int>::type = 0>
        ...
        {
            return GuiFactory::new_gui_object<InputText>(std::forward<Args>(args)...);
        }
    };
