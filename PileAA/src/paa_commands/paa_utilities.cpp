...
namespace paa {
    ...
    static inline T& internal_paa_setup_base_and_get(PAA_ENTITY entity, Args&&... args) {
        hl::silva::registry& registry = PAA_ECS;
        registry.emplace<T>(entity, std::forward<Args>(args)...);
        return registry.get_component<T>(entity);
    }

    paa::AnimatedSprite& internal_paa_set_sprite(PAA_ENTITY entity, const std::string& name) {
        hl::silva::registry& registry = PAA_ECS;
        registry.emplace<AnimatedSprite>(entity, name)
                .emplace_r<Depth>(0);
    ...
    ControllerKeyboard& internal_paa_setup_base_keyboard(PAA_ENTITY entity) {
        ...
        auto& controller = internal_paa_setup_base_and_get<ControllerKeyboard>(entity);
