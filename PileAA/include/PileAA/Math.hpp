...
struct Math {
    ...
    template <typename V> static float direction_to_angle(V const& pos)
    {
    ...
    {
        return paa::Vector2f(origin.x - direction.x, origin.y - direction.y);
    }

    template <typename V1, typename V2>
    static paa::Vector2f linear_interpolation(
        V1 const& origin, V2 const& direction, float t, float scale = 1)
    {
        return paa::Vector2f(origin.x + (direction.x - origin.x) * t * scale,
    ...
    {
        return paa::Vector2f(
            x, x < 0.5 ? 2 * x * x : 1 - std::pow(-2 * x + 2, 2) / 2);
    }
    ...
    {
        return paa::Vector2f(
            x, x < 0.5 ? 4 * x * x * x : 1 - std::pow(-2 * x + 2, 3) / 2);
    }
    ...
    {
        return paa::Vector2f(
            x, x < 0.5 ? 8 * x * x * x * x : 1 - std::pow(-2 * x + 2, 4) / 2);
    }
    ...
    {
        return paa::Vector2f(x,
            x < 0.5 ? 16 * x * x * x * x * x : 1 - std::pow(-2 * x + 2, 5) / 2);
    }
    ...
    {
        return paa::Vector2f(x,
            x < 0.5 ? (1 - std::sqrt(1 - std::pow(2 * x, 2))) / 2
    ...
    static paa::Vector2f easeOutBack(const float x)
    {
        ...
        return paa::Vector2f(
            x, 1 + c3 * std::pow(x - 1, 3) + c1 * std::pow(x - 1, 2));
    }
    ...
    static paa::Vector2f easeInOutBack(const float x)
    {
        ...
        return paa::Vector2f(x,
            x < 0.5
                ...
                : (std::pow(2 * x - 2, 2) * ((c2 + 1) * (x * 2 - 2) + c2) + 2)
                    / 2);
    }
    ...
    {
        return paa::Vector2f(x,
            x < 0.5 ? (1 - easeOutBounce(1 - 2 * x).y) / 2
                    : (1 + easeOutBounce(2 * x - 1).y) / 2);
    }
};
}