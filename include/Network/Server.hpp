...
#define RTYPE_UDP_BUFFER_SIZE 1024
        ...
        void start_receive()
        {
            _socket.async_read_some(boost::asio::buffer(*_recv_buffer),
                [this, recv_queue = _recv_queue, recv_buffer = _recv_buffer](
                    ...
                    if (error.value() >= 0) {
                        if (bytes_transferred) {
                            recv_queue->async_push(shared_message_info_t(
                                new message_info(std::move(*recv_buffer),
                                    bytes_transferred)));
                        }
                        start_receive();
                    } else {
                        ...
                        std::fprintf(
                            stderr, "Error while receiving UDP packet");
                    }
                });
        }
    ...
    public:
        ...
        bool udp_poll(udp_server::shared_message_info_t& message)
        {
            return _udp_server->poll(message);
        }

        void tcp_send(
            tcp_connection::shared_message_info_t message, size_t index)
        {
            _tcp_server->send(index, message);
        }

        void udp_send(udp_server::shared_message_info_t message)
        {
            _udp_server->send(message);
        }
