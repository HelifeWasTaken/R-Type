...
template <Number T, size_t N> class Vector;

template <Number T> class Vector2;
template <Number T> class Vector3;
...
public:
    ...
    template <Number... Args> Vector(const Args&... value)
    {
        _values = { static_cast<T>(value)... };
    }
    virtual ~Vector() = default;

    const T& get(const size_t position) const { return _values[position]; }
    ...
    const std::array<T, N>& unwrap() const { return _values; }

    template <Number T2> void operator+=(const Vector<T2, N>& other)
    {
    ...
    operator Vector2<T>() { return Vector2<T>(_values[0], _values[1]); }

    template <Number T2> bool operator==(const Vector<T2, N>& other) const
    {
        for (size_t i = 0; i < N; ++i) {
...
template <Number T> class Vector2 : public Vector<T, 2> {
...
public:
    ...
    template <Number T2> operator Vector2<T2>() const
    {
        return Vector2<T2>(this->x(), this->y());
    }

    operator sf::Vector2f() const { return sf::Vector2f(this->x(), this->y()); }

    operator sf::Vector2i() const { return sf::Vector2i(this->x(), this->y()); }

    operator sf::Vector2u() const { return sf::Vector2u(this->x(), this->y()); }
};

template <Number T> class Vector3 : public Vector<T, 3> {
...
{
    std::string str
        = std::string("Vector<") + std::to_string(N) + std::string(">[");
    for (size_t i = 0; i < N; ++i) {
...
template <Number T, size_t N> Vector<T, N> operator-(const Vector<T, N>& vector)
{
    Vector<T, N> result;
    for (size_t i = 0; i < N; ++i) {
...
template <Number T> Vector2<T> operator-(const Vector2<T>& vector)
{
