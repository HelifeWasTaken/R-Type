...
namespace obscur {
    ...
    void InputHandler::eventJoystickButtonPressed(const sf::Event& event)
    {
        ...
        _joystickPrivateEvent.push_back(
            { { event.joystickButton.joystickId, event.joystickButton.button }, event.type });
        if (_joystickTrace.at(event.joystickButton.joystickId).has_value())
            _joystickTrace.at(event.joystickButton.joystickId).value().addFollower(event.joystickButton.button);
        else
            _joystickTrace.at(event.joystickButton.joystickId).emplace(event.joystickButton.button);
    }
    ...
    void InputHandler::eventJoystickButtonReleased(const sf::Event& event)
    {
        ...
        _joystickPrivateEvent.push_back(
            { { event.joystickButton.joystickId, event.joystickButton.button }, event.type });
        if (_joystickTrace.at(event.joystickButton.joystickId).has_value()) {
            _joystickTraces.at(event.joystickButton.joystickId).push_back(_joystickTrace.at(event.joystickButton.joystickId).value());
            if (event.joystickButton.button == _joystickTrace.at(event.joystickButton.joystickId).value().getOrigin())
                _joystickTrace.at(event.joystickButton.joystickId).reset();
            else
                _joystickTrace.at(event.joystickButton.joystickId).value().removeFollower(event.joystickButton.button);
        }
    }
    ...
    void InputHandler::eventJoystickMoved(const sf::Event& event)
    {
        ...
        if (event.joystickMove.axis == sf::Joystick::Axis::Z || event.joystickMove.axis == sf::Joystick::Axis::R) {
            pos = getAxisZRGlobal(event.joystickMove.joystickId);
        }
        if (event.joystickMove.axis == sf::Joystick::Axis::U || event.joystickMove.axis == sf::Joystick::Axis::V) {
            pos = getAxisUVGlobal(event.joystickMove.joystickId);
        }
        if (event.joystickMove.axis == sf::Joystick::Axis::PovX || event.joystickMove.axis == sf::Joystick::Axis::PovY) {
    ...
    bool InputHandler::isKeyCombinationInput(const std::vector<sf::Keyboard::Key>& keys) const {
    ...
    bool InputHandler::isButtonCombinationInput(
            const JoystickIndex& joystick_id, const std::vector<JoystickButton>& buttons) const
    {
        for (auto iter = _joystickTraces.at(joystick_id).begin(); iter != _joystickTraces.at(joystick_id).end(); ++iter) {
    ...
    void InputHandler::setAxisDeadZone(const JoystickIndex& joystick_id, const float &radius)
    {
    ...
    Vector2<float> InputHandler::getAxisXYGlobal(const JoystickIndex& joystick_id) const
    {
    ...
    Vector2<float> InputHandler::getAxisZRGlobal(const JoystickIndex& joystick_id) const
    {
    ...
    Vector2<float> InputHandler::getAxisUVGlobal(const JoystickIndex& joystick_id) const
    {
    ...
    {
        return Vector2<float>(getAxisPosition(joystick_id, sf::Joystick::Axis::PovX),
    ...
    {
        for (auto iter = _joystickTraces.begin(); iter != _joystickTraces.end(); ++iter) {
            if (iter->checkCombination(buttons)) {
    ...
    bool Joystick::isButtonCombinationInput(const std::vector<JoystickButton> &buttons) const
    {
    ...
    {
        auto trace = InputHandler::getInstance().getJoystickInputs(_joystick_id, inputs.size());
    ...
    bool Keyboard::isButtonCombinationInput(const std::vector<JoystickButton>& buttons) const
    {
    ...
    {
        const uint8_t is_positive = _input_handler.isKeyPressedOrHeld(_axisKeys[axis * 2]);
        const uint8_t is_negative = _input_handler.isKeyPressedOrHeld(_axisKeys[axis * 2 + 1]);
    ...
    Vector2<float> Keyboard::getAxisXYGlobal() const
    {
        return getAxisXY();
    }
    ...
    Vector2<float> Keyboard::getAxisZRGlobal() const
    {
        return getAxisZR();
    }
    ...
    Vector2<float> Keyboard::getAxisUVGlobal() const
    {
        return getAxisUV();
    }
