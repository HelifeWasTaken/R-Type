...
/**
** co authored with Mattis DALLEAU (mattis.dalleau@epitech.eu) under license
*agreement (GPLV3)
**/
...
private:
    ...
    template <typename T>
    using EventFrameInfoList = std::vector<EventFrameInfo<T>>;
    ...
    mutable std::array<std::optional<InputTrace<JoystickButton>>,
        sf::Joystick::Count>
        _joystickTrace;
    mutable std::array<std::vector<InputTrace<JoystickButton>>,
        sf::Joystick::Count>
        _joystickTraces;
...
public:
     ...
     */
    KeyState getJoystickButtonState(
        const JoystickIndex& joystick, const JoystickButton& button) const;

    /*
    // WIP
    std::vector<Input> getKeyInputs(size_t size) const;

    std::vector<Input> getJoystickInputs(const JoystickIndex& joystick, size_t
    size) const;
    */
     ...
     */
    bool isButtonCombinationInput(const JoystickIndex& joystick_id,
...
template <typename... Args> class CallbackInputHandler {
    InputHandler& _handler = InputHandler::getInstance();

    std::unordered_map<sf::Keyboard::Key,
        std::pair<InputHandler::KeyState, std::function<void(Args...)>>>
        _key_callbacks;
    std::unordered_map<JoystickButton,
        std::pair<JoystickIndex,
            std::pair<InputHandler::KeyState, std::function<void(Args...)>>>>
        _button_callbacks;
...
public:
    ...
    void addKeyCallback(const sf::Keyboard::Key& key,
        const InputHandler::KeyState& state,
        std::function<void(Args...)>&& callback)
    {
        _key_callbacks[key] = std::make_pair(state, callback);
    }

    void addButtonCallback(const JoystickIndex& joystick_id,
        const JoystickButton& button, const InputHandler::KeyState& state,
        std::function<void(Args...)>&& callback)
    {
        _button_callbacks[button]
            = std::make_pair(joystick_id, std::make_pair(state, callback));
    }
    ...
    void update(const Args&... args)
    {
        ...
        for (auto& [button, pair] : _button_callbacks) {
            if (_handler.getJoystickButtonState(pair.first, button)
                == pair.second.first)
...
public:
    ...
    virtual bool isButtonCombinationInput(
        const std::vector<JoystickButton>& buttons) const = 0;
