...
#define RTYPE_FEED_INIT_TOKEN 84
        ...
        + defined(USE_RTYPE_MAGIC_NUMBER_32_BITS)                              \
        + defined(USE_RTYPE_MAGIC_NUMBER_16_BITS)                              \
        + defined(USE_RTYPE_MAGIC_NUMBER_8_BITS)                               \
    != 1
#error                                                                         \
    "You must define one and only one of the following macros: USE_RTYPE_MAGIC_NUMBER_64_BITS, USE_RTYPE_MAGIC_NUMBER_32_BITS, USE_RTYPE_MAGIC_NUMBER_16_BITS, USE_RTYPE_MAGIC_NUMBER_8_BITS"
#endif
...
#define RTYPE_MAGIC_NUMBER 0xff
    ...
    template <typename T> static inline void dump_memory(const T& data)
    {
    ...
    {
        dump_memory(reinterpret_cast<const Byte*>(data.data()),
            data.size() * sizeof(T));
    }
