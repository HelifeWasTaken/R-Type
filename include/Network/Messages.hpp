...
namespace net {
    ...
    static inline std::unordered_map<message_code, message_type>
        message_type_map = { { message_code::DUMMY, message_type::INVALID },
            ...
            { message_code::TEXT_REQ, message_type::QUERY_MESSAGE },
            { message_code::TEXT_REP, message_type::REPLY_MESSAGE } };
    ...
    public:
        SignalMarker()
            : _code(message_code::DUMMY)
        {
        }
        SignalMarker(message_code code)
            : _code(code)
        {
        }
        ...
        static boost::shared_ptr<SignalMarker> deserialize(
        ...
        message_code code() const { return _code; }
    ...
    public:
        ...
        template <typename T>
        UpdateMessage(int16_t sid, const T& data)
            : _sid(sid)
            , _data(data.serialize())
            , _type(T::get_serializable_type())
        {
        }

        UpdateMessage(
            int16_t sid, int16_t type, const std::vector<uint8_t>& data)
            : _sid(sid)
            , _data(data)
            , _type(type)
        {
        ...
        void from(const std::vector<uint8_t>& buff)
        {
            ...
            _data = std::vector<uint8_t>(
                buff.begin() + 1 + sizeof(_type) + sizeof(_sid), buff.end());
        }

        static boost::shared_ptr<UpdateMessage> deserialize(
            const uint8_t* buffer, size_t size)
        {
        ...
        message_code code() const { return message_code::UPDATE_MSG; }

        uint16_t type() const { return _type; }

        uint16_t sid() const { return _sid; }

        const std::vector<uint8_t> data() const { return _data; }

        template <typename T> T get() { return T::deserialize(_data); }
    ...
    public:
        ...
        template <typename T>
        SyncMessage(int16_t sid, const T& data)
            : _sid(sid)
            , _data(data.serialize())
            , _type(T::get_serializable_type())
        {
        ...
        SyncMessage(int16_t sid, int16_t type, const std::vector<uint8_t>& data)
            : _sid(sid)
            , _data(data)
            , _type(type)
        {}
        ...
        static boost::shared_ptr<SyncMessage> deserialize(
            const uint8_t* buffer, size_t size)
        {
        ...
        message_code code() const { return message_code::SYNC_MSG; }
    ...
    public:
        ...
        QueryMessage(message_code code, const std::vector<uint8_t>& data)
            : _code(code)
            , _data(data)
        {
        }
        ...
        static boost::shared_ptr<QueryMessage> deserialize(
            const uint8_t* buffer, size_t size)
        {
    ...
    public:
        ...
        ReplyMessage(
            message_code code, uint8_t status, const std::vector<uint8_t>& data)
            : _code(code)
            , _status(status)
            , _data(data)
        {
        ...
        static boost::shared_ptr<ReplyMessage> deserialize(
            const uint8_t* buffer, size_t size)
        {
    ...
    class TextReplyMessage : public ReplyMessage {
    public:
        TextReplyMessage()
            : ReplyMessage(message_code::TEXT_REP, 0, std::vector<uint8_t>())
        {
        }

        TextReplyMessage(uint8_t status, const std::string& msg)
            : ReplyMessage(message_code::TEXT_REP, status,
                std::vector<uint8_t>(msg.begin(), msg.end()))
        {
        }
        ...
        static boost::shared_ptr<TextReplyMessage> deserialize(
            const uint8_t* buffer, size_t size)
        {
            auto msg = boost::make_shared<TextReplyMessage>();
            msg->from(std::vector<uint8_t>(buffer, buffer + size));
            return msg;
        }
    ...
    public:
        ...
        void from(const std::vector<uint8_t>& buff)
        {
            ...
            std::memcpy(
                &_token, buff.data() + 1 + sizeof(_playerId), sizeof(_token));
            boost::endian::big_to_native_inplace(_token);
        }

        static boost::shared_ptr<ConnectionInitReply> deserialize(
            const uint8_t* buffer, size_t size)
        {
        ...
        message_code code() const { return message_code::CONN_INIT_REP; }

        uint16_t playerId() const { return _playerId; }

        uint32_t token() const { return _token; }
    ...
    public:
        ...
        static boost::shared_ptr<DisconnectMessage> deserialize(
            const uint8_t* buffer, size_t size)
        {
            auto msg = boost::make_shared<DisconnectMessage>();
            msg->from(std::vector<uint8_t>(buffer, buffer + size));
            return msg;
        }
        ...
        message_code code() const { return message_code::DISCONNECTION; }
    private:
        uint16_t _playerId;
    };
    ...
    public:
        ...
        static boost::shared_ptr<FeedInitRequest> deserialize(
            const uint8_t* buffer, size_t size)
        {
        ...
        message_code code() const { return message_code::FEED_INIT; }
    ...
    public:
        ...
        static boost::shared_ptr<FeedInitReply> deserialize(
            const uint8_t* buffer, size_t size)
        {
        ...
        message_code code() const { return message_code::FEED_INIT_REP; }

        int32_t token() const { return _token; }

    private:
        uint32_t _token;
    };

    static inline boost::shared_ptr<IMessage> parse_message(
        const uint8_t* buffer, size_t size)
    {
        assert(size > 0);
        ...
        case message_type::QUERY_MESSAGE:
            return QueryMessage::deserialize(buffer, size);
