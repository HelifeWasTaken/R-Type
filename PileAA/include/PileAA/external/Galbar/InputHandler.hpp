...
public:
    ...
    bool isKeyPressed(sf::Keyboard::Key key) { return p_key_pressed[key]; }

    bool isKeyDown(sf::Keyboard::Key key) { return p_key_down[key]; }

    bool isKeyReleased(sf::Keyboard::Key key) { return p_key_released[key]; }

    bool mouseMoved() { return p_mouse_move_event; }
    ...
    bool mouseWheelScrolled() { return p_mouse_wheel_scrolled; }

    float getMouseWheelScrollDelta() { return p_mouse_scroll_delta; }
    ...
    {
        return p_joystick_button_pressed[std::pair<unsigned int, unsigned int>(
            joystickId, button)];
    }

    bool isJoystickButtonDown(unsigned int joystickId, unsigned int button)
    {
        return p_joystick_button_down[std::pair<unsigned int, unsigned int>(
            joystickId, button)];
    }

    bool isJoystickButtonReleased(unsigned int joystickId, unsigned int button)
    {
        return p_joystick_button_released[std::pair<unsigned int, unsigned int>(
            joystickId, button)];
    }

    float getJoystickAxisPosition(
        unsigned int joystickId, sf::Joystick::Axis axis)
    {
        return p_joystick_axis_position[std::pair<unsigned int, unsigned int>(
            joystickId, axis)];
    }
...
private:
    ...
    bool mouseEvent(const sf::Event& event)
    {
        if (event.type == sf::Event::MouseButtonPressed) {
            p_mouse_button_pressed[event.mouseButton.button]
                = not p_mouse_button_down[event.mouseButton.button];
    ...
    bool joystickEvent(const sf::Event& event)
    {
        if (event.type == sf::Event::JoystickConnected) {
            p_joystick_connected[event.joystickConnect.joystickId]
                = not p_joystick_active[event.joystickConnect.joystickId];
            ...
            std::pair<unsigned int, unsigned int> k(
    ...
    std::unordered_map<std::pair<unsigned int, unsigned int>, bool>
        p_joystick_button_pressed;
    std::unordered_map<std::pair<unsigned int, unsigned int>, bool>
        p_joystick_button_down;
    std::unordered_map<std::pair<unsigned int, unsigned int>, bool>
        p_joystick_button_released;
    std::unordered_map<std::pair<unsigned int, unsigned int>, float>
        p_joystick_axis_position;
};
}
