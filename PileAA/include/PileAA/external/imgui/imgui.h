...
IMGUI_API void ShowStackToolWindow(bool* p_open
...
IMGUI_API void StyleColorsLight(ImGuiStyle* dst
...
IMGUI_API bool IsWindowFocused(ImGuiFocusedFlags flags
...
IMGUI_API void SetScrollHereX(float center_x_ratio
    = 0.5f); // adjust scrolling amount to make current cursor position visible.
             // center_x_ratio=0.0: left, 0.5: center, 1.0: right. When using to
...
IMGUI_API bool InvisibleButton(const char* str_id, const ImVec2& size,
    ...
    = 0); // flexible button behavior without the visuals, frequently useful to
          // build custom behaviors using the public api (along with
...
IMGUI_API void ProgressBar(float fraction,
    const ImVec2& size_arg = ImVec2(-FLT_MIN, 0), const char* overlay = NULL);
IMGUI_API void
...
IMGUI_API bool SliderFloat(const char* label, float* v, float v_min,
    ...
    = 0); // adjust format to decorate the value with a prefix or a suffix for
...
IMGUI_API bool ColorButton(const char* desc_id, const ImVec4& col,
    ...
    const ImVec2& size
    = ImVec2(0, 0)); // display a color square/button, hover for details, return
                     // true when pressed.
...
IMGUI_API bool CollapsingHeader(const char* label,
    ...
    = 0); // if returning 'true' the header is open. doesn't indent nor push on
...
IMGUI_API bool Selectable(const char* label, bool selected = false,
    ...
    const ImVec2& size
    = ImVec2(0, 0)); // "bool selected" carry the selection state (read-only).
...
IMGUI_API bool MenuItem(const char* label, const char* shortcut = NULL,
    bool selected = false, bool enabled = true); // return true when activated.
...
IMGUI_API float GetColumnWidth(int column_index
...
IMGUI_API float GetColumnOffset(int column_index
    = -1); // get position of column line (in pixels, from the left side of the
...
IMGUI_API bool SetDragDropPayload(const char* type, const void* data, size_t sz,
    ...
    = 0); // type is a user defined string of maximum 32 characters. Strings
          ...
          // is copied and held by imgui. Return true when payload has been
          // accepted.
...
IMGUI_API bool IsItemHovered(ImGuiHoveredFlags flags
    = 0); // is the last item hovered? (and usable, aka not blocked by a popup,
...
IMGUI_API bool IsItemClicked(ImGuiMouseButton mouse_button
    = 0); // is the last item hovered and mouse clicked on? (**)  ==
          // IsMouseClicked(mouse_button) && IsItemHovered()Important. (**) this
          // it NOT equivalent to the behavior of e.g. Button(). Read comments
...
IMGUI_API bool IsMouseHoveringRect(const ImVec2& r_min, const ImVec2& r_max,
    bool clip
    = true); // is mouse hovering given bounding rect (in screen space). clipped
             // by current clipping settings, but disregarding of other
             // consideration of focus/window ordering/popup-block.
IMGUI_API bool IsMousePosValid(const ImVec2* mouse_pos
    = NULL); // by convention we use (-FLT_MAX,-FLT_MAX) to denote that there is
             // no mouse available
IMGUI_API bool
...
IMGUI_API ImVec2 GetMouseDragDelta(ImGuiMouseButton button = 0,
    ...
    = -1.0f); // return the delta from the initial clicking position while the
              ...
              // at least once (if lock_threshold < -1.0f, uses
              // io.MouseDraggingThreshold)
...
IMGUI_API void CaptureMouseFromApp(bool want_capture_mouse_value
    = true); // attention: misleading name! manually override
             // io.WantCaptureMouse flag next frame (said flag is entirely left
             // for your application to handle). This is equivalent to setting
...
IMGUI_API void LoadIniSettingsFromMemory(const char* ini_data,
    size_t ini_size
    = 0); // call after CreateContext() and before the first call to NewFrame()
...
IMGUI_API const char* SaveIniSettingsToMemory(size_t* out_ini_size
    = NULL); // return a zero-terminated string with the .ini data which you can
             // save by your own mean. call when io.WantSaveIniSettings is set,
             // then save data by your own mean and clear
...
enum ImGuiWindowFlags_ {
    ...
    ImGuiWindowFlags_NoResize = 1
        << 1, // Disable user resizing with the lower-right grip
    ImGuiWindowFlags_NoMove = 1 << 2, // Disable user moving the window
    ImGuiWindowFlags_NoScrollbar = 1
        << 3, // Disable scrollbars (window can still scroll with mouse or
              // programmatically)
    ImGuiWindowFlags_NoScrollWithMouse = 1
        << 4, // Disable user vertically scrolling with mouse wheel. On child
    ...
    ImGuiWindowFlags_NoBackground = 1
        << 7, // Disable drawing background color (WindowBg, etc.) and outside
              // border. Similar as using SetNextWindowBgAlpha(0.0f).
    ImGuiWindowFlags_NoSavedSettings = 1
        << 8, // Never load/save settings in .ini file
    ...
    ImGuiWindowFlags_HorizontalScrollbar = 1
        << 11, // Allow horizontal scrollbar to appear (off by default). You may
    ...
    ImGuiWindowFlags_NoFocusOnAppearing = 1
        << 12, // Disable taking focus when transitioning from hidden to visible
               // state
    ...
    ImGuiWindowFlags_AlwaysVerticalScrollbar = 1
        << 14, // Always show vertical scrollbar (even if ContentSize.y <
               // Size.y)
    ImGuiWindowFlags_AlwaysHorizontalScrollbar = 1
        << 15, // Always show horizontal scrollbar (even if ContentSize.x <
               // Size.x)
    ImGuiWindowFlags_AlwaysUseWindowPadding = 1
        << 16, // Ensure child windows without border uses style.WindowPadding
               // (ignored by default for non-bordered child windows, because
               // more convenient)
    ImGuiWindowFlags_NoNavInputs = 1
        << 18, // No gamepad/keyboard navigation within the window
    ImGuiWindowFlags_NoNavFocus = 1
        << 19, // No focusing toward this window with gamepad/keyboard
               // navigation (e.g. skipped by CTRL+TAB)
    ImGuiWindowFlags_UnsavedDocument = 1
        << 20, // Display a dot next to the title. When used in a tab/docking
    ...
    ImGuiWindowFlags_NavFlattened = 1
        << 23, // [BETA] On child window: allow gamepad/keyboard navigation to
               // cross over parent border to this child or between sibling
               // child windows.
    ImGuiWindowFlags_ChildWindow = 1
        << 24, // Don't use! For internal use by BeginChild()
    ImGuiWindowFlags_Tooltip = 1
        << 25, // Don't use! For internal use by BeginTooltip()
    ImGuiWindowFlags_Popup = 1
        << 26, // Don't use! For internal use by BeginPopup()
    ImGuiWindowFlags_Modal = 1
        << 27, // Don't use! For internal use by BeginPopupModal()
    ImGuiWindowFlags_ChildMenu = 1
        << 28 // Don't use! For internal use by BeginMenu()
...
enum ImGuiInputTextFlags_ {
    ...
    ImGuiInputTextFlags_AutoSelectAll = 1
        << 4, // Select entire text when first taking mouse focus
    ImGuiInputTextFlags_EnterReturnsTrue = 1
        << 5, // Return 'true' when Enter is pressed (as opposed to every time
              // the value was modified). Consider looking at the
              // IsItemDeactivatedAfterEdit() function.
    ImGuiInputTextFlags_CallbackCompletion = 1
        << 6, // Callback on pressing TAB (for completion handling)
    ImGuiInputTextFlags_CallbackHistory = 1
        << 7, // Callback on pressing Up/Down arrows (for history handling)
    ImGuiInputTextFlags_CallbackAlways = 1
        << 8, // Callback on each iteration. User code may query cursor
              // position, modify text buffer.
    ImGuiInputTextFlags_CallbackCharFilter = 1
        << 9, // Callback on character inputs to replace or discard them. Modify
              // 'EventChar' to replace or discard, or return 1 in callback to
              // discard.
    ImGuiInputTextFlags_AllowTabInput = 1
        << 10, // Pressing TAB input a '\t' character into the text field
    ImGuiInputTextFlags_CtrlEnterForNewLine = 1
        << 11, // In multi-line mode, unfocus with Enter, add new line with
    ...
    ImGuiInputTextFlags_Password = 1
        << 15, // Password mode, display all characters as '*'
    ImGuiInputTextFlags_NoUndoRedo = 1
        << 16, // Disable undo/redo. Note that input text owns the text data
    ...
    ImGuiInputTextFlags_CharsScientific = 1
        << 17, // Allow 0123456789.+-*/eE (Scientific notation input)
    ...
    ImGuiInputTextFlags_CallbackEdit = 1
        << 19 // Callback on any edit (note that InputText() already returns
...
enum ImGuiTreeNodeFlags_ {
    ...
    ImGuiTreeNodeFlags_AllowItemOverlap = 1
    ...
    ImGuiTreeNodeFlags_SpanAvailWidth = 1
        << 11, // Extend hit box to the right-most edge, even if not framed.
    ...
    ImGuiTreeNodeFlags_SpanFullWidth = 1
        << 12, // Extend hit box to the left-most and right-most edges (bypass
               // the indented area).
...
enum ImGuiPopupFlags_ {
    ...
    ImGuiPopupFlags_NoOpenOverExistingPopup = 1
        << 5, // For OpenPopup*(), BeginPopupContext*(): don't open if there's
              // already a popup at the same level of the popup stack
    ImGuiPopupFlags_NoOpenOverItems = 1
        << 6, // For BeginPopupContextWindow(): don't return true when hovering
              // items, only when hovering empty space
    ImGuiPopupFlags_AnyPopupId = 1
        << 7, // For IsPopupOpen(): ignore the ImGuiID parameter and test for
              // any popup.
    ImGuiPopupFlags_AnyPopupLevel = 1
        << 8, // For IsPopupOpen(): search/test at any level of the popup stack
...
enum ImGuiSelectableFlags_ {
    ...
    ImGuiSelectableFlags_DontClosePopups = 1
        << 0, // Clicking this don't close parent popup window
    ImGuiSelectableFlags_SpanAllColumns = 1
        << 1, // Selectable frame can span all columns (text will still fit in
              // current column)
    ImGuiSelectableFlags_AllowDoubleClick = 1
        << 2, // Generate press events on double clicks too
    ImGuiSelectableFlags_Disabled = 1
        << 3, // Cannot be selected, display grayed out text
    ImGuiSelectableFlags_AllowItemOverlap = 1
        << 4 // (WIP) Hit testing to allow subsequent widgets to overlap this
             // one
};
...
enum ImGuiComboFlags_ {
    ...
    ImGuiComboFlags_PopupAlignLeft = 1
        << 0, // Align the popup toward the left by default
    ImGuiComboFlags_HeightSmall = 1
        << 1, // Max ~4 items visible. Tip: If you want your combo popup to be a
...
enum ImGuiTabBarFlags_ {
    ...
    ImGuiTabBarFlags_Reorderable = 1
        << 0, // Allow manually dragging tabs to re-order them + New tabs are
              // appended at the end of list
    ImGuiTabBarFlags_AutoSelectNewTabs = 1
        << 1, // Automatically select new tabs when they appear
    ImGuiTabBarFlags_TabListPopupButton = 1
        << 2, // Disable buttons to open the tab list popup
    ImGuiTabBarFlags_NoCloseWithMiddleMouseButton = 1
        << 3, // Disable behavior of closing tabs (that are submitted with
    ...
    ImGuiTabBarFlags_NoTabListScrollingButtons = 1
        << 4, // Disable scrolling buttons (apply when fitting policy is
              // ImGuiTabBarFlags_FittingPolicyScroll)
    ImGuiTabBarFlags_NoTooltip = 1 << 5, // Disable tooltips when hovering a tab
    ImGuiTabBarFlags_FittingPolicyResizeDown = 1
        << 6, // Resize tabs when they don't fit
    ImGuiTabBarFlags_FittingPolicyScroll = 1
        << 7, // Add scroll buttons when tabs don't fit
...
enum ImGuiTabItemFlags_ {
    ...
    ImGuiTabItemFlags_UnsavedDocument = 1
        << 0, // Display a dot next to the title + tab is selected when clicking
              ...
              // tab bar.
    ImGuiTabItemFlags_SetSelected = 1
        << 1, // Trigger flag to programmatically make the tab selected when
              // calling BeginTabItem()
    ImGuiTabItemFlags_NoCloseWithMiddleMouseButton = 1
        << 2, // Disable behavior of closing tabs (that are submitted with
              // p_open != NULL) with middle mouse button. You can still repro
              // this behavior on user's side with if (IsItemHovered() &&
              // IsMouseClicked(2)) *p_open = false.
    ...
    ImGuiTabItemFlags_NoReorder = 1
        << 5, // Disable reordering this tab or having another tab cross over
              // this tab
    ImGuiTabItemFlags_Leading = 1
        << 6, // Enforce the tab position to the left of the tab bar (after the
              // tab list popup button)
    ImGuiTabItemFlags_Trailing = 1
        << 7 // Enforce the tab position to the right of the tab bar (before the
             // scrolling buttons)
};
...
enum ImGuiTableFlags_ {
    ...
    ImGuiTableFlags_Reorderable = 1
        << 1, // Enable reordering columns in header row (need calling
              // TableSetupColumn() + TableHeadersRow() to display headers)
    ImGuiTableFlags_Hideable = 1
        << 2, // Enable hiding/disabling columns in context menu.
    ...
    ImGuiTableFlags_NoSavedSettings = 1
        << 4, // Disable persisting columns order, width and sort settings in
              // the .ini file.
    ...
    ImGuiTableFlags_BordersOuterH = 1
        << 8, // Draw horizontal borders at the top and bottom.
    ImGuiTableFlags_BordersInnerV = 1
        << 9, // Draw vertical borders between columns.
    ...
    ImGuiTableFlags_NoBordersInBody = 1
        << 11, // [ALPHA] Disable vertical borders in columns Body (borders will
               // always appears in Headers). -> May move to style
    ImGuiTableFlags_NoBordersInBodyUntilResize = 1
        << 12, // [ALPHA] Disable vertical borders in columns Body until hovered
    ...
    ImGuiTableFlags_SizingStretchProp = 3
        << 13, // Columns default to _WidthStretch with default weights
               // proportional to each columns contents widths.
    ImGuiTableFlags_SizingStretchSame = 4
        << 13, // Columns default to _WidthStretch with default weights all
    ...
    ImGuiTableFlags_PreciseWidths = 1
        << 19, // Disable distributing remainder width to stretched columns
    ...
    ImGuiTableFlags_NoClip = 1
        << 20, // Disable clipping rectangle for every individual columns
    ...
    ImGuiTableFlags_PadOuterX = 1
        << 21, // Default if BordersOuterV is on. Enable outer-most padding.
               // Generally desirable if you have headers.
    ImGuiTableFlags_NoPadOuterX = 1
        << 22, // Default if BordersOuterV is off. Disable outer-most padding.
    ImGuiTableFlags_NoPadInnerX = 1
        << 23, // Disable inner padding between columns (double inner padding if
...
enum ImGuiTableColumnFlags_ {
    ...
    ImGuiTableColumnFlags_Disabled = 1
        << 0, // Overriding/master disable flag: hide column, won't show in
    ...
    ImGuiTableColumnFlags_DefaultHide = 1
        << 1, // Default as a hidden/disabled column.
    ImGuiTableColumnFlags_DefaultSort = 1 << 2, // Default as a sorting column.
    ImGuiTableColumnFlags_WidthStretch = 1
        << 3, // Column will stretch. Preferable with horizontal scrolling
    ...
    ImGuiTableColumnFlags_NoHide = 1
        << 7, // Disable ability to hide/disable this column.
    ImGuiTableColumnFlags_NoClip = 1
        << 8, // Disable clipping for this column (all NoClip columns will
              // render in a same draw command).
    ImGuiTableColumnFlags_NoSort = 1
        << 9, // Disable ability to sort on this field (even if
              // ImGuiTableFlags_Sortable is set on the table).
    ImGuiTableColumnFlags_NoSortAscending = 1
        << 10, // Disable ability to sort in the ascending direction.
    ImGuiTableColumnFlags_NoSortDescending = 1
        << 11, // Disable ability to sort in the descending direction.
    ImGuiTableColumnFlags_NoHeaderLabel = 1
        << 12, // TableHeadersRow() will not submit label for this column.
               // Convenient for some small columns. Name will still appear in
               // context menu.
    ImGuiTableColumnFlags_NoHeaderWidth = 1
        << 13, // Disable header text width contribution to automatic column
               // width.
    ...
    ImGuiTableColumnFlags_IndentEnable = 1
        << 16, // Use current Indent value when entering cell (default for
               // column 0).
    ImGuiTableColumnFlags_IndentDisable = 1
        << 17, // Ignore current Indent value when entering cell (default for
               // columns > 0). Indentation changes _within_ the cell will still
               // be honored.

    // Output status flags, read-only via TableGetColumnFlags()
    ImGuiTableColumnFlags_IsEnabled = 1
        << 24, // Status: is enabled == not hidden by user/api (referred to as
               // "Hide" in _DefaultHide and _NoHide) flags.
    ImGuiTableColumnFlags_IsVisible = 1
        << 25, // Status: is visible == is enabled AND not clipped by scrolling.
    ImGuiTableColumnFlags_IsSorted = 1
        << 26, // Status: is currently part of the sort specs
...
enum ImGuiTableRowFlags_ {
    ...
    ImGuiTableRowFlags_Headers = 1
        << 0 // Identify header row (set default background color + width of its
...
enum ImGuiHoveredFlags_ {
    ...
    ImGuiHoveredFlags_ChildWindows = 1
        << 0, // IsWindowHovered() only: Return true if any children of the
              // window is hovered
    ImGuiHoveredFlags_RootWindow = 1
        << 1, // IsWindowHovered() only: Test from root window (top most parent
              // of the current hierarchy)
    ImGuiHoveredFlags_AnyWindow = 1
        << 2, // IsWindowHovered() only: Return true if any window is hovered
    ImGuiHoveredFlags_NoPopupHierarchy = 1
        << 3, // IsWindowHovered() only: Do not consider popup hierarchy (do not
    ...
    ImGuiHoveredFlags_AllowWhenBlockedByPopup = 1
        << 5, // Return true even if a popup window is normally blocking access
              // to this item/window
    ...
    ImGuiHoveredFlags_AllowWhenBlockedByActiveItem = 1
        << 7, // Return true even if an active item is blocking access to this
              // item/window. Useful for Drag and Drop patterns.
    ImGuiHoveredFlags_AllowWhenOverlapped = 1
        << 8, // IsItemHovered() only: Return true even if the position is
              // obstructed or overlapped by another window
    ImGuiHoveredFlags_AllowWhenDisabled = 1
        << 9, // IsItemHovered() only: Return true even if the item is disabled
    ImGuiHoveredFlags_NoNavOverride = 1
        << 10, // Disable using gamepad/keyboard navigation state when active,
               // always query mouse.
...
enum ImGuiDragDropFlags_ {
    ...
    ImGuiDragDropFlags_SourceNoPreviewTooltip = 1
        << 0, // By default, a successful call to BeginDragDropSource opens a
    ...
    ImGuiDragDropFlags_SourceNoDisableHover = 1
        << 1, // By default, when dragging we clear data so that IsItemHovered()
    ...
    ImGuiDragDropFlags_SourceNoHoldToOpenOthers = 1
        << 2, // Disable the behavior that allows to open tree nodes and
    ...
    ImGuiDragDropFlags_SourceExtern = 1
        << 4, // External source (from outside of dear imgui), won't attempt to
    ...
    ImGuiDragDropFlags_AcceptNoDrawDefaultRect = 1
        << 11, // Do not draw the default highlight rectangle when hovering over
               // target.
    ImGuiDragDropFlags_AcceptNoPreviewTooltip = 1
        << 12, // Request hiding the BeginDragDropSource tooltip from the
...
enum ImGuiKey_ {
    ...
    = ImGuiKey_NamedKey_BEGIN // First key stored in KeysData[0]
#else
...
enum ImGuiConfigFlags_ {
    ...
    ImGuiConfigFlags_NavEnableGamepad = 1
        << 1, // Master gamepad navigation enable flag. This is mostly to
    ...
    ImGuiConfigFlags_NavNoCaptureKeyboard = 1
        << 3, // Instruct navigation to not set the io.WantCaptureKeyboard flag
...
enum ImGuiBackendFlags_ {
    ...
    ImGuiBackendFlags_HasMouseCursors = 1
        << 1, // Backend Platform supports honoring GetMouseCursor() value to
              // change the OS cursor shape.
    ImGuiBackendFlags_HasSetMousePos = 1
        << 2, // Backend Platform supports io.WantSetMousePos requests to
    ...
    ImGuiBackendFlags_RendererHasVtxOffset = 1
        << 3 // Backend Renderer supports ImDrawCmd::VtxOffset. This enables
...
enum ImGuiButtonFlags_ {
    ...
    ImGuiButtonFlags_MouseButtonLeft = 1
        << 0, // React on left mouse button (default)
...
enum ImGuiColorEditFlags_ {
    ...
    ImGuiColorEditFlags_NoAlpha = 1
        << 1, //              // ColorEdit, ColorPicker, ColorButton: ignore
              //              Alpha component (will only read 3 components from
              //              the input pointer).
    ImGuiColorEditFlags_NoPicker = 1
        << 2, //              // ColorEdit: disable picker when clicking on
              //              color square.
    ImGuiColorEditFlags_NoOptions = 1
        << 3, //              // ColorEdit: disable toggling options menu when
              //              right-clicking on inputs/small preview.
    ImGuiColorEditFlags_NoSmallPreview = 1
        << 4, //              // ColorEdit, ColorPicker: disable color square
    ...
    ImGuiColorEditFlags_NoBorder = 1
        << 10, //              // ColorButton: disable border (which is enforced
               //              by default)

    // User Options (right-click on widget to change some of them).
    ImGuiColorEditFlags_AlphaBar = 1
        << 16, //              // ColorEdit, ColorPicker: show vertical alpha
               //              bar/gradient in picker.
    ImGuiColorEditFlags_AlphaPreview = 1
        << 17, //              // ColorEdit, ColorPicker, ColorButton: display
    ...
    ImGuiColorEditFlags_PickerHueBar = 1
        << 25, // [Picker]     // ColorPicker: bar for Hue, rectangle for
               // Sat/Value.
    ImGuiColorEditFlags_PickerHueWheel = 1
        << 26, // [Picker]     // ColorPicker: wheel for Hue, triangle for
               // Sat/Value.
    ImGuiColorEditFlags_InputRGB = 1
        << 27, // [Input]      // ColorEdit, ColorPicker: input and output data
               // in RGB format.
    ImGuiColorEditFlags_InputHSV = 1
        << 28, // [Input]      // ColorEdit, ColorPicker: input and output data
               // in HSV format.
    ...
    ImGuiColorEditFlags_DefaultOptions_ = ImGuiColorEditFlags_Uint8
...
enum ImGuiSliderFlags_ {
    ...
    ImGuiSliderFlags_AlwaysClamp = 1
        << 4, // Clamp value to min/max bounds when input manually with
              // CTRL+Click. By default CTRL+Click allows going out of bounds.
    ImGuiSliderFlags_Logarithmic = 1
        << 5, // Make the widget logarithmic (linear otherwise). Consider using
    ...
    ImGuiSliderFlags_NoRoundToFormat = 1
        << 6, // Disable rounding underlying value to match precision of the
              // display format string (e.g. %.3f values are rounded to those 3
              // digits)
    ImGuiSliderFlags_NoInput = 1
        << 7, // Disable CTRL+Click or Enter key allowing to input text directly
              // into the widget
...
enum ImGuiCond_ {
    ...
    ImGuiCond_FirstUseEver = 1
        << 2, // Set the variable if the object/window has no persistently saved
              // data (no entry in .ini file)
    ImGuiCond_Appearing = 1
        << 3 // Set the variable if the object/window is appearing after being
...
struct ImGuiStyle {
    ...
    float TabMinWidthForCloseButton; // Minimum width for close button to
                                     // appears on an unselected tab when
                                     // hovered. Set to 0.0f to always show when
                                     // hovering, set to FLT_MAX to never show
    ...
    ImVec2
        DisplayWindowPadding; // Window position are clamped to be visible
                              // within the display area or monitors by at least
...
struct ImGuiIO {
    ...
    bool
        ConfigWindowsResizeFromEdges; // = true           // Enable resizing of
                                      ...
                                      // flag)
    bool
        ConfigWindowsMoveFromTitleBarOnly; // = false       // Enable allowing
    ...
    void (*SetPlatformImeDataFn)(
        ImGuiViewport* viewport, ImGuiPlatformImeData* data);
#ifndef IMGUI_DISABLE_OBSOLETE_FUNCTIONS
    void* ImeWindowHandle; // = NULL           // [Obsolete] Set
                           // ImGuiViewport::PlatformHandleRaw instead. Set this
                           // to your HWND to get automatic IME cursor
                           // positioning.
#else
    ...
    IMGUI_API void AddMouseWheelEvent(
        float wh_x, float wh_y); // Queue a mouse wheel update
    ...
    IMGUI_API void AddInputCharacterUTF16(
        ImWchar16 c); // Queue a new character input from an UTF-16 character,
    ...
    int KeyMap[ImGuiKey_COUNT]; // [LEGACY] Input: map of indices into the
                                ...
                                // are now unused and should be kept zero.
                                // Legacy backend will write into KeyMap[] using
                                // ImGuiKey_ indices which are always >512.
    bool KeysDown[ImGuiKey_COUNT]; // [LEGACY] Input: Keyboard keys that are
                                   // pressed (ideally left in the "native"
                                   // order your engine has access to keyboard
                                   // keys, so you can use your own
                                   // defines/enums for keys). This used to be
                                   // [512] sized. It is now ImGuiKey_COUNT to
                                   // allow legacy io.KeysDown[GetKeyIndex(...)]
                                   // to work without an overflow.
#endif
    ...
    ImGuiKeyData KeysData[ImGuiKey_KeysData_SIZE]; // Key state for all known
                                                   // keys. Use IsKeyXXX()
...
enum ImDrawListFlags_ {
    ...
    ImDrawListFlags_AntiAliasedLines = 1
        << 0, // Enable anti-aliased lines/borders (*2 the number of triangles
    ...
    ImDrawListFlags_AntiAliasedLinesUseTex = 1
        << 1, // Enable anti-aliased lines/borders using textures when possible.
              // Require backend to render with bilinear filtering (NOT
              // point/nearest filtering).
    ImDrawListFlags_AntiAliasedFill = 1
        << 2, // Enable anti-aliased edge around filled shapes (rounded
              // rectangles, circles).
...
struct ImDrawList {
    ...
    //   and AddNgonFilled() functions if you need to guaranteed a specific
    //   number of sides.
    ...
    IMGUI_API void AddConvexPolyFilled(
        const ImVec2* points, int num_points, ImU32 col);
    IMGUI_API void AddBezierCubic(const ImVec2& p1, const ImVec2& p2,
        const ImVec2& p3, const ImVec2& p4, ImU32 col, float thickness,
        int num_segments = 0); // Cubic Bezier (4 control points)
    IMGUI_API void AddBezierQuadratic(const ImVec2& p1, const ImVec2& p2,
    ...
    inline void PathStroke(
        ImU32 col, ImDrawFlags flags = 0, float thickness = 1.0f)
    {
...
enum ImFontAtlasFlags_ {
    ...
    ImFontAtlasFlags_NoPowerOfTwoHeight = 1
        << 0, // Don't round the height to next power of two
    ImFontAtlasFlags_NoMouseCursors = 1
        << 1, // Don't build software mouse cursors into the atlas (save a
...
struct ImFontAtlas {
    ...
    // in 1.72+ typedef ImFontGlyphRangesBuilder GlyphRangesBuilder; // OBSOLETED
    // in 1.67+
};
...
struct ImFont {
    ...
    ImWchar EllipsisChar; // 2     // out // = '...'    // Character used for
                          // ellipsis rendering.
    ImWchar
        DotChar; // 2     // out // = '.'      // Character used for ellipsis
    ...
    ImU8 Used4kPagesMap[(IM_UNICODE_CODEPOINT_MAX + 1) / 4096
        / 8]; // 2 bytes if ImWchar=ImWchar16, 34 bytes if ImWchar==ImWchar32.
              // Store 1-bit for each block of 4K codepoints that has one active
              // glyph. This is mainly used to facilitate iterations across all
              // used codepoints.
    ...
    IMGUI_API void RenderText(ImDrawList* draw_list, float size,
        const ImVec2& pos, ImU32 col, const ImVec4& clip_rect,
    ...
    IMGUI_API void AddGlyph(const ImFontConfig* src_cfg, ImWchar c, float x0,
        float y0, float x1, float y1, float u0, float v0, float u1, float v1,
        float advance_x);
...
enum ImGuiViewportFlags_ {
    ...
    ImGuiViewportFlags_IsPlatformMonitor = 1
        << 1, // Represent a Platform Monitor (unused yet)
    ImGuiViewportFlags_OwnedByApp = 1
        << 2 // Platform Window: is created/managed by the application (rather
...
struct ImGuiViewport {
    ...
    {
        return ImVec2(
            WorkPos.x + WorkSize.x * 0.5f, WorkPos.y + WorkSize.y * 0.5f);
    }
};
...
static inline int GetKeyIndex(ImGuiKey key)
{
    IM_ASSERT(key >= ImGuiKey_NamedKey_BEGIN && key < ImGuiKey_NamedKey_END
        && "ImGuiKey and native_index was merged together and native_index is "
           "disabled by IMGUI_DISABLE_OBSOLETE_KEYIO. Please switch to "
           "ImGuiKey.");
    return key;
}
#endif
...
static inline bool ListBoxHeader(
...
// SetCursorPosX(GetCursorPosX() + GetTreeNodeToLabelSpacing()); }   // OBSOLETED
// in 1.72 (from July 2019) static inline void  SetNextTreeNodeOpen(bool open,
// ImGuiCond cond = 0) { SetNextItemOpen(open, cond); }                       //
