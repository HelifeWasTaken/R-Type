...
TilesetManager::TilesetManager(const std::string& filename)
{
    ...
    for (const auto& it : json["tilesets"]) {
        ...
        if (tileset->texture.loadFromFile(
                filenameNoEnding + "/" + (std::string)it["image"])
            == false) {
            throw std::runtime_error(
                "Failed to load tileset: " + (std::string)it["image"]);
        }
    ...
    for (auto& it : json["layers"]) {
            ...
            if (data[i] != 0) {
                for (size_t j = 0; j < _tilesets.size(); ++j) {
                    if ((pureGid >= _tilesets[j]->firstGid)
                        && (pureGid < _tilesets[j]->firstGid
                                + _tilesets[j]->tileCount)) {
                        tilesetIndex = j;
                        break;
                    }
                }
        ...
        auto layer = std::unique_ptr<std::pair<Texture&, VertexArray>>(
            new std::pair<Texture&, VertexArray>(
                _tilesets[tilesetIndex]->texture, VertexArray()));
        ...
        for (uint64_t i = 0; i < width; i++) {
            for (uint64_t j = 0; j < height; j++) {
                ...
                int tileNumber
                    = (rawId & ~(FLIP_H_FLAG | FLIP_V_FLAG | FLIP_D_FLAG))
                    - _tilesets[tilesetIndex]->firstGid;
                Vertex* vertices = &layer->second[(i + j * width) * 4];

                int tu = tileNumber
                    % (layer->first.getSize().x
                        / _tilesets[tilesetIndex]->tileWidth);
                int tv = tileNumber
                    / (layer->first.getSize().x
                        / _tilesets[tilesetIndex]->tileWidth);

                vertices[0].position
                    = Vector2f(i * _tilesets[tilesetIndex]->tileWidth,
                        j * _tilesets[tilesetIndex]->tileHeight);
                vertices[1].position
                    = Vector2f((i + 1) * _tilesets[tilesetIndex]->tileWidth,
                        j * _tilesets[tilesetIndex]->tileHeight);
                vertices[2].position
                    = Vector2f((i + 1) * _tilesets[tilesetIndex]->tileWidth,
                        (j + 1) * _tilesets[tilesetIndex]->tileHeight);
                vertices[3].position
                    = Vector2f(i * _tilesets[tilesetIndex]->tileWidth,
                        (j + 1) * _tilesets[tilesetIndex]->tileHeight);
                ...
                if (tileNumber < 0) {
                    ...
                    vertices[0].texCoords
                        = Vector2f(tu * _tilesets[tilesetIndex]->tileWidth,
                            tv * _tilesets[tilesetIndex]->tileHeight);
                    vertices[1].texCoords = Vector2f(
                        (tu + 1) * _tilesets[tilesetIndex]->tileWidth,
                        tv * _tilesets[tilesetIndex]->tileHeight);
                    vertices[2].texCoords = Vector2f(
                        (tu + 1) * _tilesets[tilesetIndex]->tileWidth,
                        (tv + 1) * _tilesets[tilesetIndex]->tileHeight);
...
size_t TilesetManager::layerCount() const { return _layers.size(); }
}